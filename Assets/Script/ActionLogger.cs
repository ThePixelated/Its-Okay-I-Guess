...
public class ActionLogger : MonoBehaviour
{
    ...
    public void AddLog(string source, string action, CopingTag tag, string impact, bool significant)
    {
        if (!significant && string.IsNullOrEmpty(action)) return;

        LogEntry newEntry = new LogEntry(source, action, tag, impact, significant);
